...
function buildFormData(fields = {}, files = {}) {
    ...
    if (files.photo_profile && fs.existsSync(files.photo_profile)) {
        const buffer = fs.readFileSync(files.photo_profile);
        const blob   = new Blob([buffer], { type: 'image/png' });
        form.append('photo_profile', blob, path.basename(files.photo_profile));
    }

    if (files.carte_identite && fs.existsSync(files.carte_identite)) {
        const buffer = fs.readFileSync(files.carte_identite);
        const blob   = new Blob([buffer], { type: 'image/png' });
        form.append('carte_identite', blob, path.basename(files.carte_identite));
    }
    ...
    describe('➕ POST /api/benevoles — CRÉATION', () => {
        ...
        it('devrait créer un bénévole avec TOUS les champs et Fichiers (201)', async () => {
            const form = buildFormData({
                ...
                photo_profile: PHOTO_TEST,
                carte_identite: CARTE_TEST
            });
