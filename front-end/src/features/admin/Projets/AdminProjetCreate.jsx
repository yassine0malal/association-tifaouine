...
export default function AdminProjetCreate() {
    ...
    const handleSubmit = async (e) => {
        ...
        // 1. Validation (Logique inchangée)
        const requiredFields = [
            "domaine_id", "titre_fr", "titre_ar", "titre_en",
            "description_fr", "description_ar", "description_en",
            "statut", "localisation", "budget", "date_debut", "date_fin"
        ];
        ...
        if (!imagePrincipale || !isImageValid || !isValid) {
        ...
        try {
            ...
            // 1. Champs texte (inchangé)
            Object.keys(formData).forEach(key => {
                if (key === "partenariat_ids") {
                    formData.partenariat_ids.forEach(id => {
                        dataToSend.append("partenariat_ids[]", id);
                    });
                } else {
            ...
            // 2. Fichiers : UTILISEZ LES NOMS ATTENDUS PAR LE BACKEND
            // Le backend attend 'imagePrincipale' (singulier)
            dataToSend.append("imagePrincipale", imagePrincipale.file);

            // Le backend attend 'extraImages' (le nom défini dans Multer au backend)
            ...
            // Si l'erreur est 401, c'est que le cookie n'est pas arrivé au serveur
            if (error.response?.status === 401) {
                setFormError("Session invalide. Essayez de vous reconnecter.");
            } else {
                ...
                <div className={styles.metaGrid}>
                    ...
                    <div className={styles.fieldGroup}>
                        ...
                        <div className={styles.iconInput}>
                            ...
                            </svg>                            <input type="number" className={styles.input} name="budget" placeholder="Montant" value={formData.budget} onChange={handleChange} />
                        </div>
                    </div>
