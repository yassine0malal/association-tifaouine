...
function ApplicationForm({ path, styles }) {
  ...
  // Create validation rules that update when language changes
  ...
  // Function to validate a single field and return error message
  const getFieldError = useCallback((name, value, rules = validationRules) => {
  ...
  // Update all existing errors when language changes
  useEffect(() => {
    ...
    Object.keys(formData).forEach((field) => {
      // Only check fields that currently have errors or have been touched
      if (errors[field] || touchedFields[field]) {
        const fieldValue = formData[field];
        const newError = getFieldError(field, fieldValue);
  ...
  }, [i18n.language, validationRules, getFieldError, formData, errors, touchedFields]);
    ...
    if (type === "file") {
      ...
      if (file) {
        ...
        if (!allowed.includes(file.type)) {
          setErrors((prev) => ({
            ...prev,
            [name]: t("applicationForm.validation.fileType", { types: allowed.join(", ") }),
          }));
          return;
        }
        ...
        if (file.size > maxSize) {
          setErrors((prev) => ({
            ...prev,
            [name]: t("applicationForm.validation.fileSize", { size: maxSize / (1024 * 1024) }),
          }));
          return;
        }
  ...
  const validateForm = () => {
    ...
    const requiredFields = ["fullname", "email", "phone_number", "address", "motivation"];
    ...
    if (path === "volunteer") {
      ...
      if (!formData.availability) {
        newErrors.availability = t("applicationForm.validation.availability");
        isValid = false;
      }
    }
    ...
    // File validations
    if (!formData.photo) {
      newErrors.photo = t("applicationForm.validation.photo");
      isValid = false;
    }

    if (!formData.identity_card) {
      newErrors.identity_card = t("applicationForm.validation.idCard");
      isValid = false;
    }
    ...
    <form onSubmit={handleSubmit} noValidate>
      ...
      <div className={styles.inputFields}>
        <div className={styles.twoInputs}>
          <div className={`${styles.wrapper} ${errors.fullname ? styles.error : ""}`}>
          ...
          <div className={`${styles.wrapper} ${errors.email ? styles.error : ""}`}>
        ...
        {path === "volunteer" ? (
          <div className={styles.twoInputs}>
            <div className={`${styles.wrapper} ${errors.phone_number ? styles.error : ""}`}>
            ...
            <div className={`${styles.wrapper} ${errors.skills ? styles.error : ""}`}>
        ...
        {path === "volunteer" && (
          <div className={`${styles.wrapper} ${errors.availability ? styles.error : ""}`}>
        ...
        <div className={`${styles.wrapper} ${errors.address ? styles.error : ""}`}>
        ...
        <div className={`${styles.wrapper} ${errors.motivation ? styles.error : ""}`}>
        ...
        <div className={styles.twoInputs}>
          <div className={`${styles.wrapper} ${errors.photo ? styles.error : ""}`}>
          ...
          <div className={`${styles.wrapper} ${errors.identity_card ? styles.error : ""}`}>
            ...
            {errors.identity_card && (
              <span className={styles.errorMessage}>{errors.identity_card}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <button type="submit" disabled={isSubmitting}>
        {isSubmitting ? t("applicationForm.button.submitting") : t("applicationForm.button.submit")}
      </button>
