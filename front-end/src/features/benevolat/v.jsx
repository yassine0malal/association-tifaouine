...
import styles from "./volunter-page.module.css";

function ApplicationForm({ path = "" }) {
  ...
  const validationRules = {
    ...
    phone_number: {
      ...
      message:
        "Please enter a valid Moroccan phone number (+2126XXXXXXXX or 06XXXXXXXX)",
    },
    address: {
      regex: /^.{5,100}$/,
    ...
    motivation: {
      regex: /^.{20,500}$/,
      message: "Please tell us more about your motivation (min 20 characters)",
    },
    skills: {
      regex: /^.{2,100}$/,
      message: "Please enter your skills (min 2 characters)",
    },
    availability: {
      regex: /.+/,
    ...
    if (type === "file") {
      ...
      if (file) {
        ...
        if (!allowed.includes(file.type)) {
          setErrors((prev) => ({
            ...prev,
            [name]: `File type not allowed. Allowed: ${allowed.join(", ")}`,
          }));
          return;
        }

        // Validate file size (max 5MB for photos, 10MB for others)
        const maxSize = name === "photo" ? 5 * 1024 * 1024 : 10 * 1024 * 1024;
        if (file.size > maxSize) {
          setErrors((prev) => ({
            ...prev,
            [name]: `File too large. Max ${maxSize / (1024 * 1024)}MB`,
          }));
          return;
        }
  ...
  const validateForm = () => {
    ...
    for (const field of requiredFields) {
      if (!formData[field] || !formData[field].trim()) {
        ...
        if (rule && !rule.regex.test(formData[field])) {
    ...
    if (path === "volunteer") {
      ...
      } else if (!validationRules.skills.regex.test(formData.skills)) {
        newErrors.skills = validationRules.skills.message;
      }
    ...
    // File validations
    if (!formData.photo) {
      newErrors.photo = "Please upload a face photo";
    }

    if (!formData.identity_card) {
      newErrors.identity_card = "Please upload your ID card";
    }
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className={styles.inputFields}>
          <div className={styles.twoInputs}>
            <div
              className={`${styles.wrapper} ${errors.fullname ? styles.error : ""}`}
            >
            ...
            <div
              className={`${styles.wrapper} ${errors.email ? styles.error : ""}`}
            >
          ...
          {path === "volunteer" ? (
            <div className={styles.twoInputs}>
              <div
                className={`${styles.wrapper} ${errors.phone_number ? styles.error : ""}`}
              >
              ...
              <div
                className={`${styles.wrapper} ${errors.skills ? styles.error : ""}`}
              >
                ...
                <input
                  ...
                  onChange={handleChange}
                  onBlur={() => validateField("skills", formData.skills)}
                  placeholder="e.g. Teaching, Gardening, IT"
                  className={errors.skills ? styles.inputError : ""}
                />
                {errors.skills && (
                  <span className={styles.errorMessage}>{errors.skills}</span>
                )}
              </div>
            </div>
          ) : (
          ...
          {path === "volunteer" && (
            <div
              className={`${styles.wrapper} ${errors.availability ? styles.error : ""}`}
            >
          ...
          <div
            className={`${styles.wrapper} ${errors.address ? styles.error : ""}`}
          >
          ...
          <div
            className={`${styles.wrapper} ${errors.motivation ? styles.error : ""}`}
          >
            ...
            <textarea
              ...
              onChange={handleChange}
              onBlur={() => validateField("motivation", formData.motivation)}
              placeholder="Tell us why you want to join Tifaouine..."
          ...
          <div className={styles.twoInputs}>
            <div
              className={`${styles.wrapper} ${errors.photo ? styles.error : ""}`}
            >
            ...
            <div
              className={`${styles.wrapper} ${errors.identity_card ? styles.error : ""}`}
            >
              <label htmlFor="identity_card">ID CARD (RECTO/VERSO) *</label>
              <input
                type="file"
                id="identity_card"
                name="identity_card"
        ...
        {path === "member" && (
          <div className={styles.wrapper}>
            <label htmlFor="cv_doc">CV or Portfolio (Optional)</label>
            <input
              type="file"
              id="cv_doc"
              name="cv_doc"
